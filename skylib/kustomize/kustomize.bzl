...
{}
    ...
    for inattr in ctx.attr.srcs:
        ...
        for infile in inattr.files.to_list():
            statements += ("echo $TARGET_DIR/{gitops_path}/{gitops_directory_structure}/{file}\n" +
                           "mkdir -p $TARGET_DIR/{gitops_path}/{gitops_directory_structure}\n" +
                           "echo '# GENERATED BY {rulename} -> {gitopsrulename}' > $TARGET_DIR/{gitops_path}/{gitops_directory_structure}/{file}\n" +
                           "{template_engine} --template={infile} --variable=NAMESPACE={namespace} --stamp_info_file={info_file} >> $TARGET_DIR/{gitops_path}/{gitops_directory_structure}/{file}\n").format(
                infile = "${RUNFILES}/%s" % _get_runfile_path(ctx, infile),
                rulename = inattr.label,
                gitopsrulename = ctx.label,
                namespace = namespace,
                gitops_path = ctx.attr.gitops_path,
                gitops_directory_structure = _format_gitops_directory_structure(ctx.attr.gitops_directory_structure, cluster, namespace),
                cluster = cluster,
                ...
                info_file = "${RUNFILES}/%s" % _get_runfile_path(ctx, ctx.file._info_file),
            )
